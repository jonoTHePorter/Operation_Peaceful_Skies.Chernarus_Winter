...
if (isNil "fpa_ai_cachedGroups") then {
  fpa_ai_cachedGroups = [[], []] call CBA_fnc_hashCreate;
};

private _isReCache = _units isEqualType "";
if (_isReCache) then {
  _id = _units;
  _units = [fpa_ai_cachedGroups, _id] call CBA_fnc_hashGet;
} else {
...
private _existingUnits = [fpa_ai_cachedGroups, _id] call CBA_fnc_hashGet;

if (count _existingUnits == 0) exitWith {
  [fpa_ai_cachedGroups, _id, _units] call CBA_fnc_hashSet;
  true
};
...
[fpa_ai_cachedGroups, _id, _existingUnits] call CBA_fnc_hashSet;
